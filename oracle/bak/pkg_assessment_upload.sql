create or replace package edfidata.pkg_assessment_upload
as
...
/
grant execute on EDFIDATA.PKG_ASSESSMENT_UPLOAD to APEX_190100;


create or replace package body edfidata.pkg_assessment_upload as
  ...
  begin
    ...
    elsif v_test_type = 'TSI2' then   -- TSI2 tests MUST CHECK Supplemental ID AND STUDENT ID in file
    ...
    begin
		...
		exception
			when others then
				--if the db link is not open and you try to close it and error will occur. 
				null;
		end;
  ...
  begin
    ...
    exception
      ...
      when others then
        ...
        apex_error.add_error(p_message          => 'Get dblink error for ' || v_link_target_db || ' for district ' || v_district_id || ' : ' || dbms_utility.format_error_stack,
                             p_additional_info  => substr(sqlerrm, 1, 200),
                             p_display_location => apex_error.c_inline_with_field_and_notif,
                             p_page_item_name   => '');
        raise;
  ...
  is
    ...
    e_zero_rows                exception;
		s_file_name_school_year    varchar2(200);
  begin
    ...
    if t_sat_data.count = 0 then
      ...
      n_rows_with_stu_id := t_sat_data.count;
		...
		-- set file name with schoolyear
		s_file_name_school_year := replace(v_file,'_SAT_','_SAT_2022_');
    -- open output file
    f_output := utl_file.fopen(v_output_directory, s_file_name_school_year, 'w');
    ...
    for i in 1 .. t_sat_data.count loop
      ...
      if instr(t_sat_data(i).latest_assessment_date, '/') > 0 then
        utl_file.put(f_output,t_sat_data(i).latest_assessment_date || ',');
				s_admin_date:= to_char(to_date(t_sat_data(i).latest_assessment_date,'DD/MON/YY'),'YYYYMMDD');
      else
      ...
      utl_file.put(f_output,t_sat_data(i).latest_sat_ebrw || ',');
			utl_file.put(f_output,t_sat_data(i).StudentUniqueId || '-' || 'SAT' || '-' || s_admin_date);
  ...
  ---------------------------------------------------------------------*/
	...
	procedure prc_process_tsi_data_output(v_db_link          varchar2
																			 ,v_file             varchar2
  ...
  begin
	    ...
	    begin
                ...
                s_tsi_writeplacer
				 ...
				 where file_name != v_file
					 ...
					 and (s.STUDENT_ID = t_tsi_data(i).StudentUniqueId
    ...
    -- set file name with schoolyear
		s_file_name_school_year := replace(v_file,'_TSI_','_TSI_2022_');
    -- open output file
    f_output := utl_file.fopen(v_output_directory, s_file_name_school_year, 'w');
    -- write file header to server (maybe get column names from ASSESSMENT_STANDARD_FORMAT table?)
    ...
    for i in 1 .. t_tsi_data.count loop
      ...
      if instr(t_tsi_data(i).test_start, '/') > 0 then
        utl_file.put(f_output,t_tsi_data(i).test_start || ',');
				s_admin_date:= to_char(to_date(t_tsi_data(i).test_start,'DD/MON/YY'),'YYYYMMDD');
      else
      ...
      utl_file.put(f_output,t_tsi_data(i).tsi_writeplacer || ',');
			utl_file.put(f_output,t_tsi_data(i).StudentUniqueId || '-' || 'TSI' || '-' || s_admin_date);
      utl_file.put(f_output,chr(10));
  ...
  begin
    s_sql_select_data := 'select
      ...
      from edfidata.district_tsi2_data s 
			join edfi.v_StudentIds@' || v_db_link || ' stu ' ||
      ' on file_name = ' || '''' || v_file || '''' ||
      ' and (coalesce(s.supplemental_id,s.student_id, ''0'') = stu."StudentUniqueId"
			   or coalesce(s.supplemental_id,s.student_id, ''0'') = stu."LocalCode")
      group by stu."StudentUniqueId",s.FIRST_NAME,s.LAST_NAME,s.BIRTH_DATE, test_start
        union
      select
       stu."StudentUniqueId"
    ...
    if t_tsi2_data.count = 0 then
      ...
      execute immediate s_sql_close_db_link;
		...
		-- set file name with schoolyear
		s_file_name_school_year := replace(v_file,'_TSI2_','_TSI2_2022_');
    -- open output file
    f_output := utl_file.fopen(v_output_directory, s_file_name_school_year, 'w');
    -- write file header to server (maybe get column names from ASSESSMENT_STANDARD_FORMAT table?)
    ...
    for i in 1 .. t_tsi2_data.count loop
      ...
      if instr(t_tsi2_data(i).test_start, '/') > 0 then
        utl_file.put(f_output,t_tsi2_data(i).test_start || ',');
				s_admin_date:= to_char(to_date(t_tsi2_data(i).test_start,'DD/MON/YY'),'YYYYMMDD');
      else
      ...
      utl_file.put(f_output,t_tsi2_data(i).tsi2_elar_essay || ',');
			utl_file.put(f_output,t_tsi2_data(i).StudentUniqueId || '-' || 'TSI2' || '-' || s_admin_date);
      utl_file.put(f_output,chr(10));
      utl_file.fflush(f_output);     -- add flush to ensure all recs written
    end loop;  -- end output to file loop
  ...
  begin
    ...
    for i in 1 .. t_act_data.count loop
	    begin
                ...
                s_act_scale_scores_science
				 ...
				 where file_name != v_file
					 ...
					 and (s.OTHER_ID = t_act_data(i).StudentUniqueId
		...
		-- set file name with schoolyear
		s_file_name_school_year := replace(v_file,'_ACT_','_ACT_2022_');
    -- open output file
    f_output := utl_file.fopen(v_output_directory, s_file_name_school_year, 'w');
  ...
  begin
    ...
    -- set file name with schoolyear
		s_file_name_school_year := replace(v_file,'_AP_','_AP_2022_');
    -- open output file
    f_output := utl_file.fopen(v_output_directory, s_file_name_school_year, 'w');
    -- write file header to server (maybe get column names from ASSESSMENT_STANDARD_FORMAT table?)
  ...
  begin
    ...
    -- set file name with schoolyear
		s_file_name_school_year := replace(v_file,'_IB_','_IB_2022_');
    -- open output file
    f_output := utl_file.fopen(v_output_directory, s_file_name_school_year, 'w');
    -- write file header to server (maybe get column names from ASSESSMENT_STANDARD_FORMAT table?)
